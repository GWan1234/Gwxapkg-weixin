...
`v2.8.1` 是一次面向真实微信缓存包的稳定性与性能修复版本。它保留完整的原始运行时证据，新增快速还原路径，并修复新版 WCC / CodeSpace 模板、分包路径和组件路径导致的卡住、文件缺失或输出越界问题。
...
- 默认保留 `app-config.json`、`app-service.js`、`app-wxss.js` 等解包原始文件
- `app-config.json` 作为微信编译运行时配置证据不再被清理；同时继续生成可读的 `app.json`

### 2. 快速还原模式

- 新增 `-fast`，适用于快速获取目录、原始代码、配置与基础页面结构
- 快速模式跳过 JS 美化、敏感扫描、语义重写、路由/业务面/Doctor 等审计后处理
- 大型多分包缓存包可显著缩短处理时间；需要安全审计与格式化源码时继续使用默认完整模式
...
# 指定缓存目录与输出目录快速解包
gwxapkg -id=<AppID> -in=<缓存目录> -out=<输出目录> -fast
```

### 3. WXML / CodeSpace 稳定性

- 支持加载新版 CodeSpace / WCC 运行时登记的 WXML 模板
- WXML 生成函数增加单次执行上限和整批时间预算，避免模拟运行时中的无限循环或递归阻塞 CLI
- 不能在模拟上下文中生成 DOM 的动态模板会记录为跳过项，不再阻塞其余文件的恢复

### 4. 输出目录与分包路径修复

- 修复分包根目录与解包相对路径匹配失败后回退到当前工作目录的问题
- 修复页面组件路径中的 `../` 造成 JSON/WXML 等生成文件落到 `-out` 上一级的问题
- 所有恢复路径均归一化并约束在指定输出目录内；非法越界路径会被跳过

### 5. 配置兼容性

- 缺少 `entryPagePath` 不再触发异常
- 保留仅在 `subPackages.pages` 中声明的页面，并兼容 `global.window` / `global.usingComponents`
- 对分包、组件和页面配置路径进行去重与越界校验

---

## 验证

- 已通过 `go test ./... -count=1`、`go vet ./...` 和跨平台 Windows amd64 构建
- 已在本机微信缓存中验证扫描、快速解包和完整解包流程；完整模式包含格式化与审计后处理，耗时明显高于 `-fast` 属预期行为
